...
// 查看下一个字符是否为期望的，如果是就读进，返回true，否则返回false
...
static void parseUnicodeCodePoint(Parser* parser, ByteBuffer* buf) {
    ...
    while(idx++ < 4) {
        ...
        if(parser->curChar >= '0' && parser->curChar <= '9') {
            ...
            LEX_ERROR(parser, "invlaid unicode!");
        }
    ...
    // 为代码通用，下面会直接写buf->datas，再此写入byteNum个0，以保证事先有byteNum个空间
    ...
    while(true) {
        ...
        if(parser->curChar == '%') {
            if(!matchNextChar(parser, '(')) {
                LEX_ERROR(parser, "'%s' should followed by '('!");
            }
...
static void skipComment(Parser* parser) {
    ...
    skipBlanks(parser); //注释之后又可能会有空白字符
}
...
// 断言当前token为expected并读入下一 token，否则报错errMsg
...
// 由于sourceCode未必来自于文件file,可能只是个字符串
...
void initParser(VM* vm, Parser* parser, const char* file, const char* sourceCode) {
    ...
    parser->curToken.length = 0;
    parser->curToken = parser->curToken;
